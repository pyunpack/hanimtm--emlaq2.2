    ...
    def create(self, vals):
        ...
        if 'name' in vals:
            translate_name = self.env['ir.translation'].search(
                [('src', '=', res.name), ('value', '!=', ''), ('type_of_field', '=', 'name')], limit=1)

            self.env['ir.translation'].sudo().create({
                'src': vals['name'],
                'value': translate_name.value,
        ...
        if 'model_code' in vals:
            translate_model_code = self.env['ir.translation'].search(
                [('src', '=', res.model_code), ('value', '!=', ''),
                 ('type_of_field', '=', 'model_code')], limit=1)

            self.env['ir.translation'].sudo().create({
                'src': vals['model_code'],
                'value': translate_model_code.value,
                'name': 'product.template,model_code',
                'lang': 'ar_001',
                'type': 'model',
                'show_in_catalog': True,
                'product_id': res.id,
                'type_of_field': 'model_code',
                'res_id': res.id
            })
